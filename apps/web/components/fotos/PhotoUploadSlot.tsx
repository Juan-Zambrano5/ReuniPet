...
export function PhotoUploadSlot({
  ...
  return (
    <div className="flex flex-col gap-1.5" data-testid={`slot-${slotId}`}>
      <div
        className={cn(
          'relative flex aspect-square w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded border-2 border-dashed transition-colors',
          error ? 'border-destructive bg-red-50' : 'border-[#C7C9D1] bg-surface',
          ...
          <>
            ...
            {!disabled && (
              <button
                ...
                className="absolute right-1 top-1 rounded-full bg-white/90 p-1 text-text shadow hover:bg-white"
                onClick={(e) => {
                  e.stopPropagation();
                  onClear();
                }}
